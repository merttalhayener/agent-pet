...
async function main() {
  ...
  const base = { selected: process.argv.includes('--builtin') ? 'agent-pet' : 'bsod', selectedAt: now, sleepAt: now, sleeping: false, pets };
  ...
  for (const key of ['workspaceViewWorks','claudeLinkWorks','invalidLinkRejected','languageWorks','reopenWorks','shortcutReopenWorks','collapseWorks','pinWorks','appearanceWorks','snapWorks','snapOffWorks','waitingWorks','durationWorks','completionWorks','presentationWorks','soundAvailable']) assert.ok(result[key], key);
