...
async function main() {
  ...
  const pets = ['codex', 'dewey', 'fireball', 'hoots', 'bsod', 'null-signal', 'rocky', 'seedy', 'stacky'].map(id => ({ id, name: id, file: path.join(assets, files.find(f => f.startsWith(id + '-spritesheet-') && f.endsWith('.webp'))) }));
  const now = Date.now();
  ...
  const b = { id: '22222222-2222-4222-8222-222222222222', title: 'Review changes', status: 'ready', startedAt: now - 120000, finishedAt: now - 200, changedAt: now - 300, lastEventAt: now - 200 };
  const extra = process.argv.includes('--overflow') ? Array.from({ length: 10 }, (_, i) => ({ ...b, id: 'extra-' + i, title: 'Tamamlanan sohbet ' + (i + 1) })) : [];
  const base = { selected: 'bsod', selectedAt: now, sleepAt: now, sleeping: false, pets };
  ...
  assert.equal(result.rows.find(w => w.id === '22222222-2222-4222-8222-222222222222').indicator, 'check');
  assert.ok(result.visible && result.spriteLoaded && result.transparent && result.floating && !result.hidesOnDeactivate);
  ...
  for (const key of ['languageWorks','reopenWorks','shortcutReopenWorks','collapseWorks','pinWorks','appearanceWorks','snapWorks','snapOffWorks','waitingWorks','durationWorks','completionWorks','presentationWorks','soundAvailable']) assert.ok(result[key], key);
