...
async function hookFixture({changed=false,platform='darwin',publisher='merttalhayener'}={}){
 const source=await fs.readFile(path.join(__dirname,'../src/uninstall.cjs'),'utf8');
 const root='/extensions/merttalhayener.agent-pet-0.14.1-darwin-arm64',exe=root+BUNDLE_SUFFIX,signals=[],calls=[];
 const sandbox={module:{exports:{}},__dirname:root,process:{platform,kill:(...args)=>signals.push(args)},require:name=>{
  if(name==='./helper-lifecycle.cjs')return require('../src/helper-lifecycle.cjs');
  if(name==='node:fs/promises')return {readFile:async()=>JSON.stringify({publisher,name:'agent-pet',version:'0.14.1'})};
  if(name==='node:child_process')return {execFile(){}};
  if(name==='node:util')return {promisify:()=>async(cmd,args)=>{
   calls.push([cmd,args]);if(args[0]==='-axo')return {stdout:`  123 ${exe}\n  124 /extensions/merttalhayener.agent-pet-0.14.2${BUNDLE_SUFFIX}\n  125 /other/merttalhayener.agent-pet-0.14.1-darwin-arm64${BUNDLE_SUFFIX}\n  126 /extensions/other.agent-pet-0.14.1${BUNDLE_SUFFIX}\n`};
